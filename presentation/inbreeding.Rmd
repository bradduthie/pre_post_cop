...
## When to inbreed versus avoid inbreeding?
...
Is inclusive fitness from avoiding inbreeding (left side) less than inbreeding (right side)?

$$\frac{n}{2} < \frac{n}{2} (1 + r)(1 - \delta)$$

More generally, is inclusive fitness from inbreeding (right) greater than inclusive fitness from outbreeding (left)?
...
**When should we predict selection for a alleles causing pre-copulatory inbreeding avoidance or inbreeding preference? [1]**
...
- Selection against deleterious recessives
- Dominance coefficient of deleterious recessives
- Direct selection against inbreeding avoidance
- Offspring produced per female
- Maximum mates per male
- Immigrants per generation
- Female and male carrying capacity
- Allele mutation rate (forward & back)
- Effect strength of inbreeding avoidance allele
...
<span style="font-size: 18px;">
1. Duthie, AB & JM Reid (2016) *Am. Nat.* [**188**:651-667](https://bradduthie.github.io/Duthie_Reid2016_AppendixB.pdf). From [Appendix B](https://bradduthie.github.io/Duthie_Reid2016_AppendixB.pdf)
</span>
...
$\delta \approx 0.22$ for full siblings

$\delta \approx 0.12$ for half siblings

$\delta \approx 0.06$ for first cousins
...
**Selection for inbreeding avoidance comparable when males rather than females chose mates** (*Note:* Males were mate-limited)
...
Add effective population size plot to females before showing males

After briefly showing males, then move on to showing inbreeding preference

Note that inbreeding preference is predicted for females, not males

Move on to different genetic architecture where inbreeding avoidance/preference in same sim
