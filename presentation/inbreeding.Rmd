...
## Inbreeding behaviour

- What do we exect to see given inbreeding depression?
...
Describe Parker's model

- Resource and mate limited sexes
- Mate encounter, opportunity to outbreed assumed
...
inputPanel(
  sliderInput("rr", label = "Relatedness", width="100%",
              min = 0.0, max = 1, value = 0, step = 0.05), 
  sliderInput("dd", label = "Inbreeding Depression", width="100%",
              min = 0.0, max = 1, value = 0, step = 0.1)
)
renderPlot({
    par(bg = 'transparent', mar = c(0.5, 0.5, 0.5, 0.5));
    plot(x = -1:1, y = -1:1, xaxt = "n", yaxt = "n", xlab = "", ylab = "Fitness", 
         type = "n", xlim = c(-1, 1), ylim = c(-1, 1), bty = "n");
    val <- seq(from = (-1 * pi)/4, to = (1 * pi)/4, length = 1000);
    hx  <- cos(val);
    hy  <- sin(val);
    fva <- seq(from = 0, to = 1, length = 1000);
    fit <- 0.5*(1 + input$rr)*(1 - input$dd);
    sqd <- (fva - fit)*(fva - fit);
    clf <- which(sqd == min(sqd));
    fx  <- cos(val[clf]);
    fy  <- sin(val[clf]);
    points(x = hx, y = hy, lwd = 2, type = "l");
    points(x = seq(from = 0, to = 1, length = 100), y = rep(0, 100), 
           type = "l", lty = "dotted", col = "red");
    ia_box <- mbox(x0 = -1.1, x1 = -0.1, y0 = -0.9, y1 = -1.1);
    polygon(x=ia_box$x, y=ia_box$y, lwd=3, border="black", col="black");
    ip_box <- mbox(x0 = 0.1, x1 = 1.1, y0 = -0.9, y1 = -1.1);
    polygon(x=ip_box$x, y=ip_box$y, lwd=3, border="black", col="black");
    points(x = seq(from = -1, to = 1, length = 100), y = rep(-0.85, 100), 
           lwd = 6, type = "l");
    points(x = rep(0, 100), y = seq(from = -0.85, to = 0, length = 100), 
           lwd = 6, type = "l");
    text(x = -0.6, y = -1, labels = "Avoid inbreeding", cex = 1.75, col = "red");
    text(x = 0.6, y = -1, labels = "Inbreeding", cex = 1.75, col = "white");
    text(x = 0.8, y = 0.75, labels = "n", cex = 3);
    text(x = 0.8, y = -0.75, labels = "0", cex = 3);
    text(x = -1, y = 0, labels = expression(frac(n,2)), cex = 3);
    arrows(x0 = 0, x1 = fx, y0 = 0, y1 = fy, lwd = 3, col = "black");
    arrows(x0 = 0, x1 = -0.9, y0 = 0, y1 = 0, lwd = 3, col = "red");
    points(x = 0, y = 0, cex = 5, pch = 20, col = "black");
    points(x = 0, y = 0, cex = 3, pch = 20, col = "red");
})
```
...
- Parker's model
- Add a see-saw plot that weighs relatedness versus inbreeding depression

$$\frac{n}{2} (1 + r)(1 - \delta) > \frac{n}{2}$$

Is inclusive fitness from inbreeding (left) greater than inclusive fitness from outbreeding?
...
## Interpreting Parker's model

Some key takeaways:

- Inbreeding avoidance is predicted given a sufficiently high threshold of inbreeding depression
- Inbreeding depression thresholds differ between females and males causing sexual conflict
- Females should prefer inbreeding below their inbreeding depression threshold (resource limited):
- Males should *tolerate* inbreeding below their threshold, but *never prefer* [1] (sis will be fine)

<hr>

Puurtinen's paper

## Extensions of Parker's model

## Limitations of this modelling approach

## Individual-based modelling

- Much different assumptions
- Stochasticity
- See if we can get a similar answer
...
<center>
![](images/ibm_general_1.png)
</center>

## Individual interactions

<center>
![](images/ibm_general_2.png)
</center>

## Individual interactions

<center>
![](images/ibm_general_3.png)
</center>

## General individual-based model assumptions
...
- **When should we predict selection for a mutant allele causing pre-copulatory inbreeding avoidance or inbreeding preference? [1]**
