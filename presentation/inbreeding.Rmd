...
renderPlot({
  ...
  kval <- as.matrix(kval);
  kval <- kval[,-1];
  diag(kval) <- diag(kval)/2;
  hist(kval);
  #size <- hist(gen$dat$ID)[1];
  #if(5 < 6){
  #    plot(x = 0:10, y = 0:10, type = "n", ylim = c(0, 400), cex.axis = 1.5,
  #         cex.lab = 1.5, xlab = "Generation", ylab = "k", bg = "transparent");
  #    points(x = 2, y = size, pch = 20, cex = 1.5, type = "b",
  #           lwd = 2);
  #}#else{
#      x0 <- last - 5;
#      x1 <- last + 5;
